/*
  Erro: Possível deadlock (consumidor nunca libera produtor. )
*/
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
...
void iniciaSemaforos (void) {
  ...
  nomeSemaforo = randomizaNome ("/semE", numLetras);
  sem_unlink(nomeSemaforo);
  printf("Nome semaforo: %s\n", nomeSemaforo);
  ...
  nomeSemaforo = randomizaNome ("/semP", numLetras);
  sem_unlink(nomeSemaforo);
  printf("Nome semaforo: %s\n", nomeSemaforo);
  ...
  for (int i = 0; i< numCons; i++) {
    nomeSemaforo = randomizaNome ("/semC", numLetras + i);
    sem_unlink(nomeSemaforo);
    printf("Nome semaforo: %s\n", nomeSemaforo);
